...
pub trait Highlighter: Default + 'static {
    fn highlight(&self, code: &str) -> StyledString;
}
    ...
    H: Highlighter,
{
    ...
    pub fn set_comment(&mut self, prefix: String) {
        self.comment_prefix = prefix;
    }

    pub fn comment(self, prefix: impl ToString) -> Self {
        self.with(|s| s.set_comment(prefix.to_string()))
    }
    ...
    pub fn set_cursor(&mut self, cursor: usize) {
        ...
        if focus > 0 && focus < self.rows.len() {
    ...
    fn mark_set(&mut self) {
        self.marker = Some(self.cursor);
    }

    fn comment_out(&mut self) {
        if self.cursor == 0 || self.selected_row()+1 == self.rows.len() {
            return;
        }
        ...
        if let Some(marker) = self.marker {
            ...
            if marker < self.cursor {
                start_row = self.row_at(marker);
                end_row = self.row_at(self.cursor);
            } else if self.cursor < marker {
                start_row = self.row_at(self.cursor);
                end_row = self.row_at(marker);
            } else {
                return;
            }

            let begin = self.rows[start_row].start-1;
            let end   = self.rows[end_row].end;

            let mut s = self.content[begin..end]
                .replace("\n", &(String::from("\n") + &self.comment_prefix))
                .replace(
                    &(String::from("\n") + &self.comment_prefix + &self.comment_prefix),
                    "\n",
                )
                .replace(
                    &(String::from("\n") + &self.comment_prefix + &self.comment_prefix),
            ...
            s = s.replace(&(String::from("\n") + &self.comment_prefix + "\n"), "\n\n")
            .replace(
                &(String::from("\n") + &self.comment_prefix + &self.comment_prefix),
                "\n",
            );

            self.content.replace_range(begin..end, &s);
            self.cursor = begin + s.len();
            ...
            if &self.content[row.start..row.start + self.comment_prefix.len()]
                != &self.comment_prefix
            {
                self.content.insert_str(row.start, &self.comment_prefix);
                self.cursor = max(row.end + self.comment_prefix.len(), 0);
            } else {
                self.content
                    .replace_range(row.start..row.start + self.comment_prefix.len(), "");
                self.cursor = max(row.end - self.comment_prefix.len(), 0);
            }
        }
    }

    fn copy_line_down(&mut self) {
        let selected_row = self.selected_row();
        let current_row = self.rows[selected_row];
        let current_row_str = String::from(&self.content[current_row.start..current_row.end]);
        self.content.replace_range(
            current_row.start..current_row.end,
            &(current_row_str.clone() + "\n" + &current_row_str),
        );
        self.rows.insert(selected_row + 1, current_row.clone());
        self.fix_damages();
        self.move_down();
    }
    ...
    fn cut_line(&mut self) {
        ...
        let current_row = self.rows[selected_row];
        self.content.replace_range(
            current_row.start-1..current_row.end, ""
        );
        self.cursor = current_row.start-1;
        self.rows.remove(selected_row);
    }

    fn move_line_up(&mut self) {
        let col_offset = self.col_at(self.cursor);
        ...
        let current_row = self.rows[selected_row];
        let new_row = self.rows[selected_row - 1];
        let current_row_str = String::from(&self.content[current_row.start..current_row.end]);
        let new_row_str = String::from(&self.content[new_row.start..new_row.end]);

        self.content.replace_range(
            new_row.start..current_row.end,
            &(current_row_str + "\n" + &new_row_str),
        );
        self.set_cursor(new_row.start + col_offset);
        self.fix_damages();
    }

    fn move_line_down(&mut self) {
        let negative_col_offset =
            self.rows[self.row_at(self.cursor)].width - self.col_at(self.cursor);

        let selected_row = self.selected_row();
        if selected_row + 1 == self.rows.len() {
            return;
        }
        let current_row = self.rows[selected_row];
        let new_row = self.rows[selected_row + 1];
        ...
        self.content.replace_range(
            current_row.start..new_row.end,
            &(new_row_str + "\n" + &current_row_str),
        );
        self.set_cursor(new_row.end - negative_col_offset);
        self.fix_damages();
    }
    ...
    H: Highlighter,
{
    ...
    fn draw(&self, printer: &Printer<'_, '_>) {
        printer.with_color(ColorStyle::secondary(), |printer| {
            ...
            self.scrollbase.draw(printer, |printer, i| {
                ...
                debug!("row text: `{}`", text);
                printer.with_effect(effect, |printer| {
                    printer.print_styled((0, 0), (&self.highlighter.highlight(text)).into());
                });
                ...
                if printer.focused && i == self.selected_row() {
                    ...
                    printer.print_styled((offset, 0), (&self.highlighter.highlight(c)).into());
                }
            });
        });
    }
    ...
    fn on_event(&mut self, event: Event) -> EventResult {
        ...
        match event {
            ...
            Event::AltChar(',') => self.mark_set(),
            Event::AltChar('/') => self.comment_out(),
